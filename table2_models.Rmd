---
title: "Benchmarking Models 2 - Uninformed Priors"
...
```{r, include=FALSE, echo=FALSE}
#dropped Lhh_wealth_asinh
per_cap_consumption_Bayesmodel <-
  brm(formula = consumption_asinh | weights(samp_wgt) ~
        cost_deviation + treat_any + treat_GK +
        consumption_asinh_R1 + Lvill_eligible_ratio + 
        (1 | block) + (1 | vid),
     data = per_cap_consumption_data,
     family = gaussian("identity"),
     seed = 1272022,
...
mcmc_trace(per_cap_consumption_Bayesmodel, n_warmup = 0,
           pars = c("b_Intercept", "b_cost_deviation", "b_treat_any", 
                    "b_treat_GK", "b_consumption_asinh_R1",
...
mcmc_dens(per_cap_consumption_Bayesmodel,pars = c("b_treat_GK"))
mcmc_dens_overlay(per_cap_consumption_Bayesmodel,pars = c("b_treat_GK"))
...
**total household wealth**: This is the basic benchmark model utilizing `brm()` default, uninformed priors. This model has ESS errors and divergent chains. Consider removing Lhh_wealth_asinh due to perfect collinearity issues.
...
**total household wealth**: This is the basic benchmark model utilizing `brm()` default, uninformed priors. This model does not include the Lhh_wealth_asinh variable and does not have the same issues that the previous model does. This should be considered the default.
...
```{r, include=FALSE, echo=FALSE}
# #removed Lhh_wealth_asinh
# consumption_assets_bayesmodel <-
#   brm(formula = assetsconsumption_asinh | weights(samp_wgt) ~
#         cost_deviation + treat_any + treat_GK +
#         assetsconsumption_asinh_R1 + Lvill_eligible_ratio + Lroomsnumb + Ldurablesexpenditure +
#         Ldietarydi_x_Lassetscon + Lproductiv_x_Lassetscon +
#         (1 | block) + (1 | vid),
#      data = consumption_assets_data,
#      family = gaussian("identity"),
...
Diagnostics
```{r}
# trace diagnostic plot . try same diagnostics but remove "b_Lhh_wealth_asinh"
mcmc_trace(consumption_assets_bayesmodel, n_warmup = 0,
           pars = c("b_Intercept", "b_cost_deviation", "b_treat_any", "b_treat_GK",
                    "b_assetsconsumption_asinh_R1", , "b_Lvill_eligible_ratio", 
